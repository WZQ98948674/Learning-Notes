...
![](img\mybatis\编写UserMapper接口.png)
...
![](img\mybatis\动态sql语句概述.png)

##### 2.1.2动态 SQL  之<**if>** 

我们根据实体类的不同取值，使用不同的 SQL语句来进行查询。比如在 id如果不为空时可以根据id查询，如果username 不等于空时还要加入用户名作为条件。这种情况在我们的多条件组合查询中经常会碰到。
...
![](img\mybatis\数据库数据.png)

测试查询操作：

   

![](img\mybatis\sql语句4.png)
