...
![](img\mybatis\多对多查询结果2.png)

#### 1.4 知识小结

MyBatis多表配置方式：

**一对一配置：使用<resultMap> / <resultMap> + <association>做配置**
...
![](img\mybatis\ssmsql.png)

##### 2.创建Maven工程

![](img\mybatis\ssm工程.png)
...
##### 1.整合思路
	...
	- 让spring容器管理mybatis的session工厂
	- 弃用mybatis的配置文件 , 只剩别名的配置了

![](img\11.png)



##### 2.将SqlSessionFactory配置到Spring容器中  applicationContext.xml
...
##### 3.扫描Mapper，让Spring容器产生Mapper实现类   applicationContext.xml

```xml
<!--配置Mapper扫描, 为mapper创建实现类-->
...
##### 4.配置声明式事务控制   applicationContext.xml
